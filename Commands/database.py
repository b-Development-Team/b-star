...
async def MAIN(message, args, level, perms, SERVER):
	...
	if args[1].lower() == "entries": # `tc/db entries` Displays, adds or removes entries of a table
		...
		if args[3].lower() == "remove":
			...
			conditions = {
			    arguments[z * 2]: arguments[z * 2 + 1]
			    for z in range(len(arguments) // 2)
			}
		...
		if args[3].lower() == "edit":
			...
			conditions = {
			    searching_arguments[z * 2]: searching_arguments[z * 2 + 1]
			    for z in range(len(searching_arguments) // 2)
			}
			entry = {
			    updating_arguments[z * 2]: updating_arguments[z * 2 + 1]
			    for z in range(len(updating_arguments) // 2)
			}
