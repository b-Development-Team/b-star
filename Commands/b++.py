...
def HELP(PREFIX):
	return {
	    ...
	    3,
	    "MAIN":
	    "Allows you to write short tags and/or programs",
	    "FORMAT":
	    "[subcommand]",
	    "CHANNEL":
	    0,
	    "USAGE":
	    """Using `tc/b++ run [code]` allows you to run `[code]` as B++ source code. Using `tc/b++ info \x1f\t\t(page)` displays a paged list of all B++ programs by use count, while using `tc/b++ info (program)` \x1f\t\tdisplays information and the source code of a specific program. `tc/b++ create [program] [code]` can be used \x1f\t\tto save code into a specific program name, which can be edited by its creator with `tc/b++ edit [program] \x1f\t\t[newcode]` or deleted with `tc/b++ delete [program]`. Finally, `tc/b++ [program] (args)` allows you to run any \x1f\t\tsaved program.^n^n\x1f\t\tThe full documentation for all B++ program functionality is displayed in this document:^n\x1f\t\thttps://docs.google.com/document/d/1o9uy-HBtwD5g584S_F_2dtTj6j_LGtTK16Lg1jUBgzg/edit?usp=sharing\x1f\t\t"""
	    .replace("\n", "").replace("\t", "").replace("^n", "\n"),
	    "CATEGORY":
	    "Fun",
	}
